...
export default function UsersAdmin(){
  ...
  async function load(){
    setBusy(true); setErr('');
    const t=await token(); if(!t){router.replace('/login');return;}
    const r=await fetch('/api/admin/users',{headers:{Authorization:`Bearer ${t}`}}); const d=await r.json();
  ...
  async function create(e){
    ...
    const t=await token();
    const r=await fetch('/api/admin/users',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${t}`},body:JSON.stringify(form)});
