...
export default function StorageSettings(){
  ...
  async function test(){setBusy(true);setResult(null);const {data}=await supabase.auth.getSession();const t=data?.session?.access_token;const r=await fetch('/api/box/test',{headers:{Authorization:`Bearer ${t}`}});const d=await r.json();setBusy(false);setResult({ok:r.ok,...d});}
