...
export default function Dashboard(){
  ...
  if(!ready) return <div className="loader"><img src="/miran-energy-logo.png" alt="Miran Energy"/><span>Loading Action Tracker Pro…</span></div>;

  return <main className="app-shell">
    <aside className="sidebar">
      <div className="side-brand"><img src="/miran-energy-logo.png" alt="Miran Energy"/></div>
      <nav>
        {[
          ["Dashboard",LayoutDashboard,"/"],["Actions",ListTodo,"/"],["My Actions",UserRound,"/"],["Calendar",Calendar,"/"],
          ["Reports",BarChart3,"/"],["Documents",FolderOpen,"/"],["Users",Users,"/admin/users"],["Projects",Building2,"/"],["Settings",Settings,"/settings/storage"]
        ].map(([name,Icon,path],i)=><button key={name} onClick={()=>router.push(path)} className={i===0?"nav-active":""}><Icon size={18}/><span>{name}</span></button>)}
      </nav>
