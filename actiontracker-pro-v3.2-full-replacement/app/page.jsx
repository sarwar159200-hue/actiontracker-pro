...
import {AlertCircle,CalendarDays,CheckCircle2,Circle,ListTodo,LayoutDashboard,UserRound,Calendar,BarChart3,FolderOpen,Users,Building2,Settings,Bell,Plus,Filter} from "lucide-react";
import {supabase} from "../lib/supabase";

const NAV=[
  ["/","Dashboard",LayoutDashboard],
  ["/actions","Actions",ListTodo],
  ["/my-actions","My Actions",UserRound],
...
export default function Dashboard(){
  ...
  useEffect(()=>{(async()=>{
    ...
    const [{data:p},{data:a}]=await Promise.all([
      supabase.from("profiles").select("full_name,system_role").eq("id",session.user.id).maybeSingle(),
  ...
  return <main className="shell">
    <aside className="sidebar"><img src="/miran-energy-logo.png" className="brand-img"/>
      <nav>{NAV.map(([href,name,Icon])=><Link href={href} key={href} className={href==="/"?"active":""}><Icon size={17}/>{name}</Link>)}</nav>
      <div className="side-footer">{profile?.full_name||"User"}<small>{(profile?.system_role||"user").replaceAll("_"," ")}</small></div>
    </aside>
    <section className="main"><header><div><small>MIRAN ENERGY</small><h1>Action Tracker Pro</h1></div><div className="header-actions"><button><Bell size={16}/></button><button><CalendarDays size={16}/>{new Date().toLocaleDateString("en-GB")}</button><button><Filter size={16}/>Filters</button><Link href="/actions" className="new"><Plus size={16}/>New Action</Link></div></header>
    <div className="content">
      ...
      <section className="panel"><div className="panel-title"><strong>{active==="overdue"?"Overdue Actions":active==="due"?"Due Soon":active==="total"?"Total Actions":active[0].toUpperCase()+active.slice(1)} ({shown.length})</strong><Link href="/actions">View All</Link></div>
        <table><thead><tr><th>Action ID</th><th>Action</th><th>Due Date</th><th>Days</th><th>Priority</th><th>Status</th></tr></thead>
        <tbody>{shown.length?shown.slice(0,8).map(a=><tr key={a.id}><td>{a.action_number}</td><td>{a.title}</td><td>{a.current_due_date||"—"}</td><td>{a.days===null?"—":Math.abs(a.days)}</td><td>{a.priority}</td><td>{a.status}</td></tr>):<tr><td colSpan="6" className="empty">No actions found.</td></tr>}</tbody></table>
      </section>
      <div className="dash-grid"><section className="panel empty-box">Actions by Status<br/><span>No action data yet</span></section><section className="panel empty-box">Actions by Priority<br/><span>No priority data yet</span></section><section className="panel empty-box">Recent Completed Actions<br/><span>No completed actions yet</span></section><section className="panel empty-box">Completion Trend (Last 7 Days)</section></div>
    </div></section>
  </main>
}
