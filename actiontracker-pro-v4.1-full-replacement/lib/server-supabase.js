...
export async function requireUser(request) {
  ...
  try {
    ...
    if (error || !data?.user) return { error: `Invalid login session${error?.message ? `: ${error.message}` : ""}`, status: 401 };
...
export async function requirePermission(request, permission) {
  ...
  try {
    ...
    const permissions = Array.isArray(profile.user_permissions) ? profile.user_permissions[0] : profile.user_permissions;
    if (!permissions?.[permission]) return { error: `Permission required: ${permission}`, status: 403 };
